...
external_dir <- file.path(repo_root, "data", "external", "supplementary_s2_s3", "Figure3_human_pre11_paper_id50_k1_original_style")
...
literature_priority_species <- tibble::tribble(
  ...
  "Staphylococcus epidermidis", "Skin/Nasal Staphylococcus", "core human skin and nasal Staphylococcus; retained from v3", "Liu2026_nasal; Liu2025_skin; CMR2026_review",
  ...
  "Staphylococcus hominis", "Skin Staphylococcus", "common human skin coagulase-negative Staphylococcus; retained from v3", "Liu2025_skin; CMR2026_review",
  "Staphylococcus capitis", "Skin Staphylococcus", "common human skin coagulase-negative Staphylococcus; retained from v3", "Liu2025_skin; CMR2026_review",
  ...
  "Staphylococcus haemolyticus", "Skin Staphylococcus", "literature-supported human skin Staphylococcus; added in v4 despite n<5", "Liu2025_skin; CMR2026_review",
  "Staphylococcus lugdunensis", "Skin Staphylococcus", "clinically relevant human skin Staphylococcus; added in v4 despite n<5", "Liu2025_skin; CMR2026_review",
  ...
  "Staphylococcus pettenkoferi", "Skin Staphylococcus", "human skin-associated Staphylococcus represented in current data; added in v4", "Liu2025_skin; CMR2026_review",
  ...
  "Cutibacterium granulosum", "Skin Cutibacterium", "human skin Cutibacterium representative; retained from v3", "Liu2025_skin; CMR2026_review",
  ...
  "Cutibacterium avidum", "Skin Cutibacterium", "human skin Cutibacterium; added in v4 despite n<5", "Liu2025_skin; CMR2026_review",
  ...
  "Corynebacterium striatum", "Skin/Nasal Corynebacterium", "human skin/nasal Corynebacterium; added in v4 despite n<5", "Liu2025_skin; Liu2026_nasal; CMR2026_review",
...
write.table(selected_species_metadata,
            file.path(plot_table_dir, "Figure3C_KeySpecies_Rationale_v4.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)
write.table(data.frame(SelectedSpecies = selected_species),
            file.path(plot_table_dir, "Figure3C_KeySpecies_Selected_v4.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)
write.table(df,
            file.path(plot_table_dir, "Figure3C_KeySpecies_AllCF_Architecture_v4.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)
...
# Keep only eCFs observed with positive copy number.
...
ann <- ggplot(annotation_df, aes(x = CF, y = annotation)) +
  ...
  scale_fill_identity() +
  labs(x = NULL, y = NULL, title = "Species-level eCF/eCM architecture among representative epithelial-associated taxa") +
...
bubble <- ggplot(plot_df, aes(x = CF, y = Species)) +
  ...
  facet_grid(. ~ functional_theme_plot, scales = "free_x", space = "free_x") +
  scale_size_continuous(name = "eCF prevalence", range = c(0.2, 4.2), limits = c(0, 1), breaks = c(0.25, 0.5, 0.75, 1)) +
  scale_color_manual(
    name = "Mean copy\n(eCF-positive MAGs)",
...
ggsave(file.path(out_dir, "Fig_S2C_species_eCF_eCM_architecture_epithelial_taxa.png"),
       combined, width = 16, height = plot_height, dpi = 300)
ggsave(file.path(out_dir, "Fig_S2C_species_eCF_eCM_architecture_epithelial_taxa.pdf"),
       combined, width = 16, height = plot_height)

message("Fig. S2C completed. Selected species: ", length(selected_species),
