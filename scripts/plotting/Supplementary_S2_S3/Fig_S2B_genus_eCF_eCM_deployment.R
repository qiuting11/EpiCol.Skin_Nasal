...
plot_df <- df %>%
  ...
  left_join(genus_summary %>% select(Genus, HabitatGroup), by = "Genus") %>%
  mutate(
...
ann <- ggplot(annotation_df, aes(x = CF, y = annotation)) +
  ...
  scale_fill_identity() +
  labs(x = NULL, y = NULL, title = "Genus-level eCF/eCM deployment in epithelial-associated MAGs") +
...
ggsave(file.path(out_dir, "Fig_S2B_genus_eCF_eCM_deployment_epithelial_MAGs.png"),
       combined, width = 16, height = plot_height, dpi = 300)
ggsave(file.path(out_dir, "Fig_S2B_genus_eCF_eCM_deployment_epithelial_MAGs.pdf"),
       combined, width = 16, height = plot_height)

message("Fig. S2B completed. Copy color uses fixed bins: ", paste(copy_bin_levels, collapse = ", "))
