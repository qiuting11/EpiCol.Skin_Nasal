...
# Config
# -----------------------------
input_cf_coverage_pos <- "data/figure4/cf_phylum_coverage_long.csv"   # positive-only long table
...
mk <- function(df, title, subtitle){
  ggplot(df, aes(x = CF_id, y = percentage)) +
    ...
    scale_fill_manual(values = my_colors, labels = function(x) sub("^module", "eCM", x), name = "eCM") +
    scale_color_manual(values = my_colors, labels = function(x) sub("^module", "eCM", x), name = "eCM") +
    scale_size_continuous(range = c(0.8, 4.8), name = "Phylum size") +
    labs(title = title, subtitle = subtitle, x = "eCF", y = "Species coverage in phylum (%)") +
...
p_full <- mk(full, "Figure4B (MMseqs): Full coverage distribution", "Includes zero-coverage phylum-eCF combinations")
