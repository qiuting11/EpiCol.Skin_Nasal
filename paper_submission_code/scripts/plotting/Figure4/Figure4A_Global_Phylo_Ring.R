...
# Config
# -----------------------------
input_tree <- "E:/data/mmseqs/sp/02_Reference/gtdb_r226_phylum_reduced.tree"
input_phylum_size <- "E:/data/mmseqs/sp/04_Final_Metrics/phylum_size.csv"
input_cf_coverage <- "E:/data/mmseqs/sp/04_Final_Metrics/cf_phylum_coverage_long.csv"
...
p <- p +
  geom_fruit(
    ...
    axis.params = list(axis = "x", text.angle = 90, text.size = 1.2, vjust = 0.5),
...
p <- p +
  geom_fruit(
    ...
    axis.params = list(axis = "x", title = "Mean CFs", title.size = 2, text.size = 1.5)
  ) +
...
message("Missing CF list: ", out_missing_cf)
