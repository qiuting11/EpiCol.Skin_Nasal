...
# 1. 环境准备与路径定义
script_dir <- "E:/data/R/gephe_R/scripts/Figure3_rebuild"
...
paths <- list(
  ...
  copy_counts = "E:/data/mmseqs/sp/04_Final_Metrics/true_cf_genome_copy_counts.csv",
  taxonomy = "E:/data/mmseqs/sp/02_Reference/bac120_taxonomy.tsv",
  f3b_deployment = "E:/data/R/gephe_R/results/figures/figures3_rebuild/Figure3B_Genus_AllCF_Deployment.tsv",
  out_dir = "E:/data/R/gephe_R/results/figures/Figure4_Final"
)
dir.create(paths$out_dir, recursive = TRUE, showWarnings = FALSE)

# 2. 获取与 Fig 3B 一致的属列表及其顺序
...
ann <- ggplot(annotation_df, aes(x = CF_id, y = "CM")) +
  ...
  scale_fill_identity() +
  labs(x = NULL, y = NULL, title = "Figure 4C | CF Dosage (GTDB Scale) Aligned with Fig 3B Genera") +
...
cf_richness <- plot_df %>%
  group_by(Genus) %>%
  summarise(CF_richness = n_distinct(CF_id[!is.na(copy_bin)]), .groups = "drop") %>%
  mutate(Genus = factor(Genus, levels = genus_levels))
...
p_richness <- ggplot(cf_richness, aes(x = CF_richness, y = Genus)) +
  geom_col(fill = "#4C78A8", width = 0.72) +
  scale_y_discrete(drop = FALSE) +
  labs(x = "CF detected", y = NULL) +
  track_theme
...
track_panel_2 <- make_track_header("CF Count") / p_richness + plot_layout(heights = c(0.55, body_height))
