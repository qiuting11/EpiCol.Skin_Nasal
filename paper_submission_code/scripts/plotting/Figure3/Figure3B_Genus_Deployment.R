...
ann <- ggplot(annotation_df, aes(x = CF, y = annotation)) +
  ...
  scale_fill_identity() +
  labs(x = NULL, y = NULL, title = "Figure 3B | Genus-level all-CF deployment") +
...
track_panel_2 <- make_track_header("CF rich.") / p_richness + plot_layout(heights = c(0.55, body_height))
...
message("Figure3B v3 completed. Copy color uses fixed bins: ", paste(copy_bin_levels, collapse = ", "))
