...
script_dir <- "E:/data/R/gephe_R/scripts/Figure3_Final"
source(file.path(script_dir, "Figure3_color_config.R"))

in_dir <- "E:/data/R/gephe_R/results/figures/Figure3_Final"
out_dir <- in_dir
...
annotation_df <- cf_meta %>%
  transmute(
    ...
    annotation = factor("CM", levels = "CM"),
    fill_color = module_fill,
    label = str_replace(module, "module", "CM")
  )
...
ann <- ggplot(annotation_df, aes(x = CF, y = annotation)) +
  ...
  scale_fill_identity() +
  labs(x = NULL, y = NULL, title = "Figure 3C v4 | Literature-guided CF architecture of representative species") +
