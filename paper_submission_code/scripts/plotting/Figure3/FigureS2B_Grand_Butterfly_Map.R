...
# Paths
input_path <- "E:/data/R/gephe_R/results/diversification_analysis/Grand_Butterfly_AllRanks_Data.tsv"
output_dir <- "E:/data/R/gephe_R/results/figures/Figure3_Final"
...
p <- ggplot(df, aes(group = Taxon)) +
  ...
  theme_bw() +
  labs(
    title = "Figure 3D: Hierarchical Implementation of Colonization Factor Subfamilies",
...
p <- p + geom_text(
  data = df_labs, aes(x = x_pos, y = y_pos, label = Label, hjust = hjust_val),
